...
let putQest = document.getElementById("putQuestion");
...
let quest_set = questions.data[0];
let level = "easy";
let asked_quest = [];
let i=0;
getquestion();
function getquestion()
{
    ...
    questions_asked++;
    count.innerHTML = questions_asked+" of "+total_questions;
    let questionD = document.createElement('div');
if(i== 4)
{
level = "moderate";
asked_quest.length = 0;
}
if(i==7){level = "difficult";
asked_quest.length = 0;
}
num = getRandomQuestion(asked_quest);
currentIndex = level+""+questions_asked;
asked_quest.push(num);
questionD.innerHTML = quest_set[level][num].question;
putQest.innerHTML = quest_set[level][num].question;
answerO = quest_set[level][num].answer;
quest_set[level][num].options.forEach((element,index) => {
    let optionD = document.createElement('div');
    optionD.className = "ans ml-2";
    optionIndex = currentIndex+""+index;
    optionD.setAttribute("id",optionIndex);
    element.answer = answerO;
    optionD.innerHTML =  `<label class="radio"> <input type="radio" name="brazil" value="brazil"> <span>${element.option}</span>
    </label>`;//element.option;
    optionD.addEventListener("click",()=>{
    
         //cell.style.backgroundColor='blue';
         if(element.option == element.answer)
         {
             gameOver = false;
score++;
points.innerHTML = "points "+score;
             ...
             //cell.innerHTML=""
             questionD.style.transition = '1.0s';
             setTimeout(() => {
                app.innerHTML = "";   
             }, 500);
            ...
            result.innerHTML = "Great!";
             ...
             result.innerHTML = "You loose ,Press start button to restart";
             if(optionD.style.backgroundColor!="red"){
...
function getRandomQuestion(asked_quest)
{
    let randomNumber = Math.ceil(Math.random()*total_questions);
    if(!asked_quest.includes(randomNumber))
    {
         return randomNumber;
    }else{
        getRandomQuestion(asked_quest);
    }
return randomNumber;
}
